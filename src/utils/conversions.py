...
def r2g(ros_msg, msg_type) -> gtsam.Pose3:
...
def g2r(gtsam_obj:gtsam.Pose3) -> Pose:
    ...
    if isinstance(gtsam_obj, gtsam.Pose3):
        ...
        qx, qy, qz, qw = pose.rotation().toQuaternion().coeffs()
...
def n2r(numpy_arr:np.array, msg:any) -> any:
    ...
    if msg == "Image":
        ...
        return sensor_msgs_py.point_cloud2.create_cloud(header,fields,np.array(numpy_arr)) # return pc2.create_cloud(header, fields, np.array(numpy_arr))
